...
export async function cutPdf(
  ...
  // `--deterministic-id` обязателен: по умолчанию qpdf собирает /ID из времени
  // и имени выходного файла, то есть один и тот же кусок одной и той же
  // программы получал бы на каждом прогоне новый sha256. На отпечатке держатся
  // обе идемпотентности конвейера — серверная дедупликация повторно
  // загруженного источника и пропуск уже импортированного курса, — и обе молча
