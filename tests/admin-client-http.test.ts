 ...
 * Пауза между попытками увеличена вдвое против первой версии (50 мс вместо
 * 25), а число попыток вдвое уменьшено (100 вместо 200): потолок ожидания
 * (5 с) тот же, а сетевых обращений к серверу — вдвое меньше. Мокнутые
 * OCR и codex отвечают почти мгновенно, так что на исправном пути хватает
 * одной-двух попыток; частый опрос имел цену не для этого теста, а для
 * соседей, идущих с ним параллельно.
 */
...
describe('admin-client против настоящего сервера', () => {
  ...
  beforeEach(async () => {
    ...
    app = buildServer(undefined, {
      ...
      // Постановка в очередь (`enqueueSource`/`retrySource`) сама зовёт
      // `wake()`, так что этот интервал — только подстраховка на случай
      // пропущенного пробуждения, а не то, что двигает job вперёд. 5 мс
      // означало сотни лишних `SELECT ... WHERE status = 'queued'` за время
      // одного теста; 100 мс даёт тот же результат заметно дешевле для
      // процессора, который делят с ним параллельные файлы.
      catalogWorker: { runner, autoPollMs: 100, now: () => NOW },
  ...
  // Явный срок: тест ждёт дважды через `until` (источник, затем сборка), и
  // на исправном пути это доли секунды, но потолок каждого ожидания — 5 с
  // (100 попыток × 50 мс). Сложенные потолки дают 10 с, а умолчание vitest —
  // 5 с на весь тест: без своего срока настоящий отказ (например, «долив не
  // пришёл») тонул бы в невнятном таймауте раньше, чем успевал бы дойти до
  // строки `throw new Error('не дождались: ...')`. 20 с — запас поверх 10 с
  // потолка на сетевые обращения и запись в SQLite вокруг двух ожиданий.
  it('проходит вход, курс, источник, сборку и публикацию по сети', { timeout: 20_000 }, async () => {
