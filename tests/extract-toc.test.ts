...
describe('parseArgs', () => {
  ...
  // По виду это положительное целое, но за пределом точности `Number` сходится
  // к `1e20`, а `Math.min` дальше прижимает его к числу страниц: `--scan` молча
  // означал бы «распознать учебник целиком», то есть отказ по сроку OCR, не
  // назвавший ни аргумент, ни причину. Так же проверяют `--attempts` и `--cycles`.
  it('отвергает --scan за пределом точных целых', () => {
    expect(() =>
      parseArgs(['--subject', 'math', '--pdf', 'b.pdf', '--scan', '99999999999999999999']),
    ).toThrow(/--scan/);
  });
  ...
  // Пустая строка не `undefined`: она доезжает до `resolve()` и молча означает
  // текущий каталог — оглавление легло бы не туда, куда просили.
  it('отвергает пустое значение флага', () => {
    expect(() => parseArgs(['--subject', 'math', '--pdf', 'b.pdf', '--out', '']))
      .toThrow(/пустое значение/);
    expect(() => parseArgs(['--subject', 'math', '--pdf', '   '])).toThrow(/пустое значение/);
  });
