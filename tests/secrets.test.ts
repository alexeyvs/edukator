...
describe('хранение секретов', () => {
  ...
  it('не бросает на параметрах, которые проходят по границам, но не по maxmem', () => {
    // `128 * N * r` для этой строки — ровно потолок, а сам `scrypt` берёт ещё и
    // блок распараллеливания и отвергает её по `maxmem`. Проверка по неполной
    // формуле пускала бы такую запись в KDF, и одна испорченная строка в базе
    // превращала бы каждую попытку входа этой учётной записи в пятисотку вместо
    // отказа.
    const salt = Buffer.from('соль-соль-соль').toString('base64url');
    const key = Buffer.alloc(SCRYPT_KEY_LENGTH, 1).toString('base64url');
    expect(verifySecret(`scrypt$65536$8$16$${salt}$${key}`, 'пароль1234')).toBe(false);
  });
