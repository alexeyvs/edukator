...
describe('cutPdf', () => {
  it('передаёт qpdf по одному диапазону страниц на отрезок', async () => {
    ...
    expect(argv).toEqual([
      // `--deterministic-id` держит весь конвейер идемпотентным: без него qpdf
      // собирает /ID из времени и имени выходного файла, и один и тот же кусок
      // одной и той же программы получал бы новый отпечаток на каждом прогоне.
      // Сервер дедуплицирует источник по sha256, а импорт по нему же узнаёт уже
      // опубликованный курс — оба узнавания перестали бы срабатывать вовсе.
      '--empty', '--deterministic-id', '--pages',
      input, '2-3',
      input, '9-9',
      '--', output,
    ]);
