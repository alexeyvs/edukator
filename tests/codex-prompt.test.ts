...
// Правила второго уровня — межмодульный инвариант с `taskProblems`: промпт
// объявляет их модели, валидатор проверяет. Разъехавшись, они дают ровно то,
// что случилось на проде — генератор старается выполнить одно правило, а батч
// бракуется по другому, и суточная квота сгорает вхолостую.
describe('buildGenerationPrompt: правила расширенной подсказки', () => {
  it('называет границы объёма и единственную секцию, где ответ запрещён', () => {
    const prompt = buildGenerationPrompt({ topic: topic(), difficulty: 2, persona: PERSONA });

    expect(prompt).toContain('200–350 слов');
    // Ориентир нужен отдельно от границ: модель стабильно целилась в минимум и
    // недобирала до него — 27 отбраковок за трое суток на 169–198 словах.
    expect(prompt).toContain('целься в 280');
    expect(prompt).toMatch(/Ответ текущего задания не называет только чек-лист/u);
    expect(prompt).toMatch(/прийти к тому же ответу, что и текущее задание, им можно/u);
  });
});
