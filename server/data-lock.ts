...
export function releaseDataLockOnSignals(
  ...
  for (const signal of signals) {
    process.once(signal, () => {
      ...
      // руками от отказа.
      //
      // И досылается **после** текущей рассылки, а не внутри неё. На тот же
      // сигнал подписан `runChild` (пока жив потомок codex), и он свой
      // обработчик снимает уже в этой рассылке, а сигнал себе не досылает —
      // считает, что это сделает подписавшийся раньше. Досланный синхронно
      // сигнал попадает в ещё живой сторож libuv, встаёт в очередь и приезжает
      // в `emit` уже без единого слушателя, то есть не делает ничего: замок
      // снят, а многочасовой прогрев продолжает жечь codex и суточную квоту,
      // будто Ctrl-C и не было. Отложенный уходит, когда слушателей не
      // осталось и умолчание сигнала вернулось.
      queueMicrotask(() => {
