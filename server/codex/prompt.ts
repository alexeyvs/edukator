 ...
 * строки, а два знака, которые он пропускает (U+2028 и U+2029), досылает сам
 ...
 * то есть сам себе выносил вердикт. `inlineField` от этого защищён сам: `\s+`
 * с флагом `u` оба знака уже схлопывает.
 */
...
function dataBlock(value: unknown): string {
  return JSON.stringify(value, null, 2).replace(
    LINE_SEPARATORS,
    (sign) => `\\u${sign.codePointAt(0)?.toString(16) ?? ''}`,
  );
}
