 ...
 * ученика, и «завести заново» здесь означало бы его стереть. Пропавший файл
 * `ready`-ребёнка поэтому отказ, а не повод завести пустую: строка в реестре уже
 * говорит «готов», и пустая база вернулась бы к ученику молча, с нулевым
 * прогрессом вместо потерянного.
 */
export function provisionChildDatabase(
  ...
  try {
    ...
    if (existsSync(target)) {
      // Перенос по этой ветке не идёт. Она означает «база уже на месте», а
      // доказать, что это копия исходной, а не заведённая рядом пустая, нечем:
      // молча пройдя её, перенос отчитался бы успехом, ничего не перенеся, и
      // сразу закрыл бы себе повтор — ребёнок стал бы `ready` с чужой базой.
      if (options.source !== undefined) {
        throw new Error(
          `База ребёнка ${childId} уже лежит в ${target}: перенос не может доказать, ` +
            'что это копия исходной. Проверьте её вручную',
        );
      }
    ...
    // Готовый ребёнок без файла базы — не «заводим заново», а пропажа. Пустая
    // база на его месте вернулась бы к ученику как успешное заведение, стерев
    // весь прогресс: строка `ready` при этом уже есть, и заметить подмену
    // нечем. Восстанавливать копию из бэкапа — решение человека, а не протокола.
    if (child.status === 'ready') {
      throw new Error(
        `База ребёнка ${childId} объявлена готовой, но файла ${target} нет: ` +
          'восстановите его из копии, заводить пустую поверх заведение не станет',
      );
    }
    ...
    //
    // Уже готовый ребёнок в `failed` не понижается: сюда он попадает с пропавшим
    // файлом либо с базой новее приложения, и обе беды снаружи — вернули копию,
    // откатили версию, — а `failed` пережил бы их и оставил бы ученика без
    // занятий на исправной базе.
    if (child.status !== 'ready') {
      try {
