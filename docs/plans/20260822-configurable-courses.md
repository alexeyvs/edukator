...
- [ ] расширить `server/routes/family.ts` чтением опубликованного каталога и
  текущих назначений каждого ребёнка
- [ ] добавить idempotent PUT/DELETE назначения и замену исключений с проверкой,
  что курс опубликован, тема активна, а ребёнок принадлежит родителю
- [ ] после успешного изменения инвалидировать provider; снятие назначения не
  удаляет детскую БД и не обрывает активный run
- [ ] не раскрывать черновые/архивные курсы и данные источников родителю; для
  чужого/несуществующего ребёнка сохранить одинаковый ответ
- [ ] написать API-тесты назначения всех тем, исключений, новых тем редакции,
  повторного назначения, чужого ребёнка и конкурентного обновления
- [ ] запустить `npx vitest run tests/family-routes.test.ts tests/course-assignments.test.ts tests/auth-routes.test.ts` и `npm run typecheck`
