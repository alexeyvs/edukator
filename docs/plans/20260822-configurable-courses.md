...
- [x] добавить явный идемпотентный root-скрипт установки OCRmyPDF, Tesseract
  `rus+eng`, Poppler и qpdf для поддерживаемого дистрибутива edukator.ru
- [x] добавить read-only проверку бинарников, минимальных версий и языковых
  пакетов в удалённый preflight `deploy.sh` до упаковки/остановки сервиса
- [x] не выполнять `apt install` из обычного deploy; вывести точную команду
  подготовки хоста при отсутствии зависимости
- [x] добавить maintenance-marker для первого старта мигрирующего релиза: до
  успешного health разрешены диагностика и миграции, но запрещены пользовательские
  записи
- [x] при неуспешном health останавливать новую версию, восстанавливать полный
  predeploy snapshot баз и catalog artifacts и лишь затем запускать прежний код
- [x] обновить shell-тесты успешного preflight, каждого отсутствующего пакета,
  отсутствующего `rus/eng`, maintenance-окна, полного restore и отсутствия утечки
  proxy/путей в вывод
- [x] проверить, что отказ OCR после старта деградирует только catalog worker, а
  занятия по опубликованным курсам и rollback приложения остаются рабочими
- [x] запустить `npx vitest run tests/deploy-release.test.ts tests/health.test.ts tests/run-child.test.ts` и `npm run lint`
...
- [x] обновить `README.md`: модель курсов, роли, API, каталог артефактов,
  OCR/drafting, назначения, backup и команды проверки
- [x] обновить `CLAUDE.md`: новые границы `CurriculumProvider`, схемы,
  инварианты редакций, catalog worker и обязательные тестовые гейты
- [x] документировать миграцию, объём диска, очистку failed/temp artifacts,
  ручной OCR smoke и диагностику health/admin UI
- [x] сверить документацию с фактическими именами маршрутов, переменных, команд и
  файлов; удалить устаревшие заявления про три предмета и фиксированный 7 класс
- [x] проверить `git diff --check` и отсутствие незапланированных файлов/секретов
