...
export default defineConfig({
  test: {
    environment: 'node',
    setupFiles: ['web/src/vitest-setup.ts'],
    ...
    coverage: {
      ...
      thresholds: {
        ...
        // Чистые модули импорта из ФРП: эвристика нарезки, разбор манифеста,
        // отбраковка и клиент детерминированы и подменяемы целиком. `frp-pdf` и
        // `import-frp` завязаны на внешние процессы и живой сервер — для них
        // общий порог был бы враньём, как и для `extract-toc`.
